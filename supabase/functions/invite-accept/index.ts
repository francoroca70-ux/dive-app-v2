...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // Match accepted = false OR NULL, not just false: the invites table's
    // `accepted` column has no default set at the row level in the insert
    // code (Settings -> Crew never sets it), so if the column itself
    // defaults to NULL rather than false, a plain `.eq('accepted', false)`
    // would never match ANY invite, ever, under normal SQL NULL semantics --
    // which looks exactly like "this invite is invalid" for every invite.
